...
class HelloTriangleApplication {
  ...
  private:
	...
	float progress = 0.0f;
	int totalSeconds = 610;

	std::unique_ptr<ui::UProgressBar> uProgressBar;
	...
	void mainLoop() {
		uProgressBar =
			std::make_unique<ui::UProgressBar>(progress, totalSeconds);
	...
	void drawImGui() {
		...
		uint32_t newWidth = std::max(static_cast<uint32_t>(winContentExtent.x),
			static_cast<uint32_t>(64));
		uint32_t newHeight = std::max(static_cast<uint32_t>(winContentExtent.y),
			static_cast<uint32_t>(64));
		...
		float width = displaySize.x * 0.9f;
		float offset = (displaySize.x - width) * 0.5f;
		ImGui::SetNextWindowPos(ImVec2(offset, displaySize.y - 200));
		ImGui::Begin("Hello, world!", nullptr,
			ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize |
				ImGuiWindowFlags_AlwaysAutoResize |
				ImGuiWindowFlags_NoSavedSettings);
		// progress bar
		ImVec2 progressBarSize = ImVec2(width, 40);
		uProgressBar->render(progressBarSize);
		// progress bar

		// button group
		ImGuiStyle &style = ImGui::GetStyle();
		float buttonWidth = 0.0f;
		buttonWidth += ImGui::CalcTextSize("打开").x;
		buttonWidth += style.ItemSpacing.x;
		buttonWidth += ImGui::CalcTextSize("播放").x;
		buttonWidth += style.ItemSpacing.x;
		buttonWidth += ImGui::CalcTextSize("暂停").x;
		alignForWidth(buttonWidth);
	...
	void recordCommandBuffer(
		...
		beginTriangleRenderPass(commandBuffer);
	...
	void beginTriangleRenderPass(VkCommandBuffer commandBuffer) {
	...
	void createGraphicsPipeline() {
		auto vertShaderCode = readFile("demo-shaders/vert-texture.spv");
		auto fragShaderCode = readFile("demo-shaders/frag-texture.spv");
	...
	void createCommandBuffers() {
		...
		if (vkAllocateCommandBuffers(
				device, &allocInfo, commandBuffers.data()) != VK_SUCCESS) {
